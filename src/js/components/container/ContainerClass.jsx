...
class Container extends Component {
  ...
  getData = async (startdate, enddate, comparestart, compareend, type) => {
    ...
    const datasetCall = await fetch(`https://www.ncei.noaa.gov/cdo-web/api/v2/datasets?stationid=${station}`, {
      headers: {
        token: token
      }
    });
    ...
    if (dataset) {
      ...
      const call = await fetch(`https://www.ncei.noaa.gov/cdo-web/api/v2/data?stationid=${station}&datasetid=${dataset.results[1].id}&startdate=${startdate}&enddate=${enddate}&units=standard&limit=1000&datatypeid=${type}`,
        {
          headers: {
            token: token
          }
      ...
      const compareCall = await fetch(`https://www.ncei.noaa.gov/cdo-web/api/v2/data?stationid=${station}&datasetid=${dataset.results[1].id}&startdate=${comparestart}&enddate=${compareend}&units=standard&limit=1000&datatypeid=${type}`,
        {
          headers: {
            token: token
          }
